...
public class Main_Menu : MonoBehaviour {
    ...
    public bool skip_fade_in = false;                                                   // If true, this script will skip the fade in and immediately begin accepting input. For debugging
    ...
    void Awake()
    {
        ...
        cursor_positions[0] = new Vector3(GameObject.Find("new_game").GetComponent<Transform>().position.x - 136f, GameObject.Find("new_game").GetComponent<Transform>().position.y, -5f);
        cursor_positions[1] = new Vector3(GameObject.Find("load_game").GetComponent<Transform>().position.x - 136f, GameObject.Find("load_game").GetComponent<Transform>().position.y, -5f);
        cursor_positions[2] = new Vector3(GameObject.Find("settings").GetComponent<Transform>().position.x - 136f, GameObject.Find("settings").GetComponent<Transform>().position.y, -5f);
        cursor_positions[3] = new Vector3(GameObject.Find("credits").GetComponent<Transform>().position.x - 136f, GameObject.Find("credits").GetComponent<Transform>().position.y, -5f);
        cursor_positions[4] = new Vector3(GameObject.Find("exit").GetComponent<Transform>().position.x - 136f, GameObject.Find("exit").GetComponent<Transform>().position.y, -5f);
        ...
        cursor.position = cursor_positions[0];                                          // Set cursor to top option
        ...
        if (skip_fade_in)
        {
            GameObject.Find("background_main_menu").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("new_game").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("load_game").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("settings").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("credits").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("exit").GetComponent<Make_Transparent>().enabled = false;
            GameObject.Find("main_cursor").GetComponent<Make_Transparent>().enabled = false;
        }
    }
    ...
    IEnumerator selectionMade()
    {
        ...
        {
            case 0:                                                                         // New Game
                yield return new WaitForSeconds(1.0f);                                      // Wait for a second before calling the New_Game script
                camera.move("background_new_game");                                         // Move the camera to the new game screen.
                //GameObject.Find("Functions").GetComponent<New_Game>().Activate();           // Turn over control to the New_Game script
